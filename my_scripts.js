...
function loadStatsPage()
{
	var win = 0;
	var lose = 0;
	for(var i = 2; i < document.getElementById("stats_table").rows.length;i++)
	{
		var buf = document.getElementById("stats_table").rows[i].cells[2].innerHTML;
		var opp = document.getElementById("stats_table").rows[i].cells[3].innerHTML;
		if (buf < opp)
		{
			document.getElementById("stats_table").rows[i].cells[4].innerHTML = document.getElementById("stats_table").rows[i].cells[1].innerHTML;
			lose++;
		}
		else
		{
...
{
	var player = document.getElementById("player_selector");
	var a = "";
	for(var i = 0; i < players.length; i++)
	{
		var name = players[i].name;
		a = a + "<a class = 'dropdown-item' href = '#' onclick = 'switchPlayers("+i+")'>" + name +"</a>"
	}
	player.innerHTML = a;
}
